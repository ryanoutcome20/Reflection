...
function Reflection.Counter()
    Reflection.Print("Actively protecting the server from %s registered actors!", string.Comma(table.Count(Reflection.Blacklist)))
end
...
function Reflection.CheckAllowed(Player)
    ...
    elseif BlacklistOSID then
        if Reflection.Config.Kick then
            game.KickID(SID, "[Reflection] Blacklisted: " .. oSID)
        end
